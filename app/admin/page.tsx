...
export default function AdminDashboard() {
  ...
  const handleNotifyCustomer = async (order: Order, type: "Processing" | "Shipped" | "Delivered") => {
    ...
    try {
      ...
      await updateDoc(doc(db, "orders", order.id), { notificationHistory: arrayUnion(newNotification) });
      fetchData(); 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full min-h-[80vh]">
      ...
      {/* PERFECTLY CLOSED RESPONSIVE HEADER */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b pb-6 gap-4">
        ...
        <div className="flex flex-wrap gap-3 w-full sm:w-auto">
          <Link href="/admin/add-product" className="bg-brand-rose text-white px-5 py-2.5 rounded-md hover:bg-brand-burgundy font-medium text-sm flex-grow sm:flex-grow-0 text-center whitespace-nowrap">
            + Add Product
          </Link>
          <button onClick={handleLogout} className="border px-5 py-2.5 rounded-md hover:bg-gray-50 text-sm flex-grow sm:flex-grow-0 whitespace-nowrap">
            Logout
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 sm:gap-4 mb-6">
        <button onClick={() => setActiveTab("orders")} className={`px-6 py-3 rounded-t-lg font-medium border-b-2 transition-colors ${activeTab === "orders" ? "border-brand-burgundy text-brand-burgundy bg-brand-rose/5" : "border-transparent text-gray-500 hover:text-brand-burgundy"}`}>
          Orders ({orders.length})
        </button>
        <button onClick={() => setActiveTab("inventory")} className={`px-6 py-3 rounded-t-lg font-medium border-b-2 transition-colors ${activeTab === "inventory" ? "border-brand-burgundy text-brand-burgundy bg-brand-rose/5" : "border-transparent text-gray-500 hover:text-brand-burgundy"}`}>
          Inventory ({products.length})
        </button>
      </div>

      {/* SECURELY WRAPPED CONTENT BOX (Prevents mobile cropping) */}
      <div className="bg-white rounded-xl shadow-sm border border-brand-rose/20 min-h-[400px] w-full overflow-hidden">
        {loadingData ? (
          <div className="flex justify-center items-center h-[400px] text-gray-500">Loading database records...</div>
        ) : activeTab === "orders" ? (
          orders.length === 0 ? (
            <div className="flex justify-center items-center h-[400px] text-gray-500">No orders found.</div>
          ) : (
                ...
                <tbody className="divide-y">
                  {orders.map(order => (
                    <tr key={order.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <button onClick={() => setSelectedOrder(order)} className="bg-brand-rose/10 text-brand-burgundy px-4 py-2 rounded-md hover:bg-brand-rose/20 text-sm font-medium transition-colors whitespace-nowrap">
                          View Details
                        </button>
                      </td>
                    </tr>
          ...
          products.length === 0 ? (
            <div className="flex justify-center items-center h-[400px] text-gray-500">No products found.</div>
          ) : (
            <div className="overflow-x-auto w-full">
                  ...
                  {products.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 flex items-center gap-4">
                        ...
                        <div>
                          <span className="font-medium text-brand-charcoal">{p.name}</span>
                          <span className="text-xs text-gray-400 block">{p.isActive ? 'Active' : 'Hidden'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-medium text-brand-burgundy">₹{p.price.toLocaleString("en-IN")}</td>
                      <td className="px-6 py-4 space-x-4">
                        <button onClick={() => handleToggleProduct(p.id, p.isActive)} title="Toggle Visibility">{p.isActive ? <EyeOff className="w-5 h-5 inline text-gray-500 hover:text-brand-burgundy"/> : <Eye className="w-5 h-5 inline text-gray-500 hover:text-brand-burgundy"/>}</button>
                        <Link href={`/admin/edit-product/${p.id}`}><Edit className="w-5 h-5 inline text-blue-500 hover:text-blue-700"/></Link>
                        <button onClick={() => handleDeleteProduct(p.id)}><Trash2 className="w-5 h-5 inline text-red-500 hover:text-red-700"/></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center z-10">
              ...
              <button onClick={() => setSelectedOrder(null)} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><X className="w-5 h-5" /></button>
            </div>

            <div className="p-6 space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-brand-rose/5 p-4 rounded-lg border border-brand-rose/20">
                <div>
                  ...
                  <select 
                    ...
                    className={`w-full p-2 rounded border text-sm font-medium outline-none focus:ring-1 focus:ring-brand-rose ${selectedOrder.paymentStatus === "Verified" ? "bg-green-50 text-green-700 border-green-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                  >
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full p-2 rounded border text-sm bg-white outline-none focus:ring-1 focus:ring-brand-rose"
                  >
              ...
              <div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg flex items-start gap-4 border border-gray-100">
                  <MapPin className="text-brand-rose w-5 h-5 mt-1 flex-shrink-0" />
                  <div>
                    <p className="font-semibold text-lg text-brand-charcoal">{selectedOrder.shippingAddress.fullName}</p>
                ...
                <div className="border border-gray-200 rounded-lg divide-y">
                  {selectedOrder.items.map((item, idx) => (
                    <div key={idx} className="p-3 flex justify-between items-center bg-white text-sm">
                      <div className="text-brand-charcoal"><span className="font-medium">{item.quantity}x</span> {item.name} <span className="text-brand-rose font-medium text-xs ml-2 border border-brand-rose/20 px-2 py-0.5 rounded">Size: {item.size}</span></div>
                      <div className="font-medium text-brand-burgundy">₹{(item.price * item.quantity).toLocaleString("en-IN")}</div>
                    </div>
                  ))}
                  <div className="p-3 bg-brand-cream/30 flex justify-between items-center font-bold text-brand-burgundy">
